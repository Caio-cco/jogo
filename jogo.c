...
int arquivo_existe(){
    ...
    if((file = fopen("Pontuacao.txt","r"))!=NULL){
        fclose(file);
        return 1;
    }
    else{
...
switch (op){
	...
	opfile = arquivo_existe();
    if(opfile==1){
        file = fopen("Pontuacao.txt","a");
        fprintf(file,"%s --- %d\n",nome,pontos);
    }
    else{
        file = fopen("Pontuacao.txt","w");
        fprintf(file,"-x-x-x- Score Board -x-x-x-\nNome --- Pontos\n%s --- %d\n",nome,pontos);
    }
    fclose(file);
	break;
	...
	case 3:
		system("clear");
		printf("%50c Caio Sousa Mello\n",' ');
			printf("%50c Gabriela Tamura Borella\n",' ');
				printf("%50c Henrique Pereira De Oliveira\n",' ');
		printf("%45c com110 \n",' ');
		printf("%50c Pressione ENTER para continuar",' ');
		getchar();
		getchar();
